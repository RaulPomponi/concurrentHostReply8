...
/*
 * Concurrent 	
 * 				Programma		XXH: TC Fattura Cumulativa
 * 				Abbreviazione	TC_FATTURA_CUMULATIVA
 * 			
 * Tabella 		XXH_PREFATTURE_ESITI
 * Package		XXH_PREFATTURE_ESITI_PKG
 */
public class IntegrazioneFatturazioneCumulativa {
	public int exitStatus=0;
	private Connection    connORA;
	private Connection   connIRIS;
	private ConcurrentHostConnection chc;
	private String sqlRegistraIncassi = "select distinct NUM_PREFATTURA, TO_CHAR(DATA_INCASSO,'DD/MM/YYYY') DATA_INCASSO, NULL IMPORTO, MOD_PAG " +
			" ,NUM_FATT, RECEIPT_NUMBER,AMOUNT, TO_CHAR(DATA_FATT,'DD/MM/YYYY')  DATA_FATT, CUSTOMER_NUMBER, CUSTOMER_NAME, ORG_ID" +
			" from xxh_PREFATTURE_RegIncasso_v WHERE XXH_PREFATTURE_ESITI_PKG.IS_EXPORTED(NUM_PREFATTURA,NULL,'I')='N'" +
			" AND ORG_ID = :1";// and num_prefattura='2021PFT000040'";
	private int numFieldsIncassi = 4;
	private String sqlSetDatiOracle = "select distinct NUM_PREFATTURA, NUM_FATT, TO_CHAR(DATA_FATT,'DD/MM/YYYY') DATA_FATT,  DIV " + 
			" , CUSTOMER_NUMBER, CUSTOMER_NAME, ORG_ID" +
			" from xxh_PREFATTURE_SetDatiOracle_v WHERE XXH_PREFATTURE_ESITI_PKG.IS_EXPORTED(NUM_PREFATTURA, NUM_FATT,'F')='N'" +
			" AND ORG_ID = :1";// and num_prefattura='2021PFT000040'";
	private int numFieldsFatture = 4;
	private String tcRegistriIncassi = "Custom_DWHViews.PREFATTURE_RegistraIncasso";
	private String tcSetDatiOracle = "Custom_DWHViews.PREFATTURE_SetDatiOracle";
	private String oraRegistriIncassi = "XXH_PREFATTURE_ESITI_PKG.ESITO_INCASSI";
	private String oraSetDatiOracle = "XXH_PREFATTURE_ESITI_PKG.ESITO_FATTURE";
	private String requestId;
	private String sOrgId="81";
	private String sTIF = "T"; //T = Tutti 	-	I = Incassi		-		F = Fatture
...
//		String arg1 = "TCFatturaCumulativa FCP_REQID=21588099 FCP_LOGIN=\"APPS/ASR0m1C1pUt4098ARu8w\" FCP_USERID=5261 FCP_USERNAME=\"SB002449\" FCP_PRINTER=\"noprint\" FCP_SAVE_OUT=Y FCP_NUM_COPIES=0 \"81\" \"T\"";
		...
		IntegrazioneFatturazioneCumulativa ifc = new IntegrazioneFatturazioneCumulativa(arg0,arg1);
        int exitStatus=0;
		try {
			ifc.runRegistraIncassi();
		} catch (Exception e) {
        ...
        try {
			ifc.runSetDatiOracle();
		} catch (Exception e) {
			System.out.println("Errore Fatture");
			e.printStackTrace();
			ifc.exitStatus=2;
		}
        System.exit(ifc.exitStatus);
    }
	private void runRegistraIncassi() {
		if(sTIF ==null || "T".equalsIgnoreCase(sTIF) || "I".equalsIgnoreCase(sTIF)) {
			System.out.println("run RegistraIncassi");
			copyEbs2TC(sqlRegistraIncassi,tcRegistriIncassi,oraRegistriIncassi,numFieldsIncassi);
		}
	}
	private void runSetDatiOracle() {
		if(sTIF ==null || "T".equalsIgnoreCase(sTIF) || "F".equalsIgnoreCase(sTIF)) {
			System.out.println("run SetDatiOracle");
			copyEbs2TC(sqlSetDatiOracle,tcSetDatiOracle,oraSetDatiOracle,numFieldsFatture);
		}		
	}
	...
	public IntegrazioneFatturazioneCumulativa(String p1, String p2) {
		try {
	        ...
	        sTIF = (String)arguments.get("P_TUTTI_INCASSI_FATTURE");
