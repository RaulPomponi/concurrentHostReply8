...
public class SendReportConcurretByPec {
	private String arg0;
	private String arg1;
	...
	public static void main(String[] args) {
		SendReportConcurretByPec obj = new SendReportConcurretByPec();
		obj.setArg0(args[0]);
		obj.setArg1(args[1]);
...
//		obj.setSqlPec("SELECT MAIL, MAIL_CC, MAIL_BCC, OGGETTO, CORPO, ABSOLUTE_NOME_FILE, NOME_FILE FROM XXX_ISOLA_PEC_DOC_V");
	...
	private void run() {
		try {
			...
			setSqlCredenziali("select distinct MITTENTE, PWD, SMTP, 'Y' IS_CRIPTED from XXH.XXX_GESTIONE_ORDINIPO where org_id= "+chc.getConcurrentParameter().get("P_ORG_ID").toString());
			sqlPec = "select " + 
					"  ARGUMENT2 MAIL, " + 
					"  ARGUMENT3 MAIL_CC, " + 
					"  null MAIL_BCC , " + 
					"  ARGUMENT4 OGGETTO, " + 
					"  ARGUMENT5 CORPO, " + 
					"  (select b.file_name from fnd_conc_req_outputs b where b.concurrent_request_id=a.ARGUMENT1)  ABSOLUTE_NOME_FILE,  " + 
					"  ARGUMENT7 || '_'||to_char(ARGUMENT1)||'_1.xls' NOME_FILE  " + 
					"    from fnd_concurrent_requests a " + 
					"    where request_id = " + chc.getRequestId();
