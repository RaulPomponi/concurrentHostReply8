...
public class ReadMail extends ConcurrentProgramBase {
	...
	protected void doService() throws Exception {
      ...
      try {
         infoMail = "Select distinct replace(replace(mittente_mail,'-nomail-',null),'-no-mail-') USER_NAME,PWD_MAIL,POP3,PORTA_POP3 from xxh_gestione_mail where pop3 is not null and porta_pop3 is not null and org_id = ?";
         PinfoMail = this.conn.prepareStatement(infoMail);
		 //aggiunta SD
		 System.out.println("orgID = "+String.valueOf(orgID));
         PinfoMail.setString(1, String.valueOf(orgID));
         RinfoMail = PinfoMail.executeQuery();
       ...
       //                                      throw var200;
                                          ...
                                          try {
                                             pstmt = this.conn.prepareStatement(String.format("SELECT file_data FROM xxh_read_mail_ordini_all WHERE read_mail_ordine_id = ? and org_id = ? FOR UPDATE"));
...
//                                             throw var200;
	...
	public void setOrg() {
		...
		try {
			...
			System.out.println("ORG impostata a " + this.orgID);
		} catch (Exception e) {
			System.out.println(e.getMessage());
			System.out.println("Errore nell'impostazione della org, verra' impostata la 81.");
		}
