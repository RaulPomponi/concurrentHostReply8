...
//    	      break;
	...
	public void mergePdfCU( String inPath, String outPath) {
//		String[] starts = startsPageComma.split(",");
		int fromPage=0;
		int toPage=0;
		File folder = new File(inPath);
		File[] listOfFiles = folder.listFiles();
		String[] sList = new String[listOfFiles.length];
		
		for (int i = 0; i < listOfFiles.length; i++) {
		  if (listOfFiles[i].isFile()) {
		    System.out.println("File " + listOfFiles[i].getName());
			File pdfFile = new File(inPath+"/"+listOfFiles[i].getName());
			try {
				PDDocument pdfDocument = PDDocument.load(pdfFile);
				int numPag = pdfDocument.getPages().getCount();
				Splitter splitter = new Splitter();
				...
				PDDocument pdfDocPartial = lst.get(0);
				String fileName = outPath+"/TEMP_CU_" + orgId+"_"+i +".pdf";
				sList[i]=fileName;
				File f = new File(fileName);
				pdfDocPartial.save(f);
				pdfDocPartial.close();
		...
		try {
			...
			for(int i=0;i<sList.length;i++) {
					ut.addSource(sList[i]);
			}
			ut.setDestinationFileName(outPath+"/CU_" + suffissoCU.substring(0,suffissoCU.length()-1) +".pdf");
			ut.mergeDocuments();
		} catch (Exception e) {
		...
		try {
			for(int i=0;i<sList.length;i++) {
				Files.deleteIfExists(Paths.get(sList[i]));
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
